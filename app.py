...
# NOTE: hardcoding a real API key in source is risky if this file is ever shared,
# committed to a repo, or deployed publicly. This reads from Streamlit secrets
# first (Settings -> Secrets on Streamlit Cloud, or a local
# .streamlit/secrets.toml with ZENROWS_API_KEY = "..."), and falls back to this
# default so the app still runs out of the box. You can also override it in the
# sidebar at runtime.
...
# 2-letter country codes for ZenRows' proxy_country parameter, so the request
# is geolocated the same way a real shopper on that store would be.
PROXY_COUNTRY_MAP = {
    "Saudi Arabia": "sa",
    "UAE": "ae"
}
...
def extract_skus_from_json(node, found=None):
    """Recursively walk a parsed JSON response and collect any SKU-like
    values, no matter how deeply or differently they're nested.
 
    Noon's internal catalog API is undocumented and its response shape can
    vary between endpoints/categories or change over time. Rather than
    betting everything on one or two guessed key paths (which silently
    return nothing if the guess is wrong), this walks the whole structure
    looking for known SKU field names, so it keeps working even if the
    surrounding structure shifts.
    """
    if found is None:
        found = []
